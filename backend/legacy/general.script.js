/*** Legacy data processing script -- strongly deprecated ***/
...
function exportSubject() {
  ...
  data = filterDuplicate(data.map((each) => each.subject));
  fs.writeFile(filePath + fileName, JSON.stringify(data), (error) =>
    handleErrorExportJSON(error, fileName),
  );
}
