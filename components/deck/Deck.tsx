...
function Deck({ courses }: { courses: readonly Course[] }) {
  return (
    <div className="w-[97.5%] mx-auto pb-4 overflow-x-auto flex flex-row space-x-4 whitespace-nowrap">
      {courses.map((course) => (
...
function Card({ course }: { course: Course; key }) {
  return (
    <div className="w-[300px] min-w-[300px] bg-gray-100 shadow text-black p-4 rounded-md">
      <div className="basis-1/6 font-medium text-lg rounded-lg">
        <h2>{course.code}</h2>
      </div>
      <div className="basis-full pb-2 font-normal text-base h-7 line-clamp-1 text-wrap overflow-hidden text-ellipsis">
